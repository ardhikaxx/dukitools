...
export default function WordToPdfWorkspace({ tool }: { tool: ToolConfig }) {
  ...
  async function handleConvert() {
    if (!file) {
      toast.warning('Pilih file Word terlebih dahulu.');
      return;
    }
  ...
  if (state.status === 'processing') {
    return <LoadingSpinner label="Mengonversi Word ke PDF..." />;
  }

  if (state.status === 'success' && state.result) {
    return (
      <ResultCard
        title="Word Berhasil Dikonversi ke PDF"
    ...
    <div className="space-y-4">
      {!file ? (
        <FileUpload
          ...
          label="Seret & letakkan file Word di sini, atau klik untuk memilih"
        />
      ) : (
      ...
      {file && (
        <Button onClick={handleConvert} variant="primary" fullWidth>Konversi ke PDF</Button>
      )}
    </div>
  );
}
