...
export default function PowerpointToPdfWorkspace({ tool }: { tool: ToolConfig }) {
  ...
  async function handleConvert() {
    if (!file) {
      toast.warning('Pilih file PowerPoint terlebih dahulu.');
      return;
    }
    ...
    try {
      ...
      const res = await fetch('/api/office/convert', { method: 'POST', body: formData });
    ...
    <div className="space-y-4">
      {!file ? (
        <FileUpload
          ...
          label="Seret & letakkan file PowerPoint di sini, atau klik untuk memilih"
        />
      ) : (
